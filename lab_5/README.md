...
В качестве второй функции была взята $$g(x) = (1 + key) \ mod \ (size - 1))$$

Томас Кормен в своей книге «Алгоритмы: построение и анализ» привел такую функцию что при размере таблицы $$m = 2^n$$ и правильном выборе хэш-функции, метод двойного хэширования превосходит методы линейного и квадратичного поиска и близко к идеальной схеме равномерного хэширования. Но в моем случае не получатся такой размер хэш-таблицы, так как я ставлю четкий размер под определенный load_factor.

Для квадратичного хэширования были взяты коэффициенты 0.5, тем самым шаг равен i.
