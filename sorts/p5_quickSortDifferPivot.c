...
//----------------------------------------------------------------------------------
//----------------------------------------------------------------------------------
int medianOfThree_random(int* arr, int low, int high) {
    assert(arr);
    int mid1 = low + rand() % (high - low + 1);
    int mid2 = low + rand() % (high - low + 1);
    int mid3 = low + rand() % (high - low + 1);

    return medianOfThree(arr, medianOfThree(arr, mid1, mid2), mid3);
}
...
int choose_pivot(int* arr, int low, int high, int method_pivot) {
    ...
    switch (method_pivot) {
        ...
        case 4: // Медиана трех случайных
            return medianOfThree_random(arr, low, high);
...
//----------------------------------------------------------------------------------
//----------------------------------------------------------------------------------
void sortCenterElement(int* arr, size_t n) {
    assert(arr);
    quicksort(arr, 0, n - 1, 1);
}

void sortMedianOfThree(int* arr, size_t n) {
    assert(arr);
    quicksort(arr, 0, n - 1, 2);
}

void sortRandomElement(int* arr, size_t n) {
    assert(arr);
    quicksort(arr, 0, n - 1, 3);
}

void sortMedianOfThreeRandom(int* arr, size_t n) {
    assert(arr);
    quicksort(arr, 0, n - 1, 2);
}
